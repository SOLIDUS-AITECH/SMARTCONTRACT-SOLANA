...
import { deposit, getConfigPda, getProgramTokenVaultPda, pause, PaymentGpuMarketplace } from "../sdk";
import { boilerPlateReduction } from "./utils/helpers";
import { AITECH_TOKEN, DENOMINATION, GPU_RENTER_ACCOUNT_KEYPAIR, OWNER_KEYPAIR } from "./utils/consts";
...
describe("02. Payment: deposit unit test", async function () {
  ...
  let configPda: PublicKey, configData: Awaited<ReturnType<typeof PaymentContract.account.config.fetch>>;
  ...
  it("TC-101: deposit success: correct transfer amounts", async () => {
    ...
    const preStakingBalance = (await getAccount(provider.connection, configData.stakingWallet)).amount;
    const preUserBalance = (await getAccount(provider.connection, userTokenAccount.address)).amount;
    ...
    const { instruction: depositIx } = await deposit(PaymentContract, GPU_RENTER_ACCOUNT_KEYPAIR.publicKey, { amountAitech: new BN(depositAmount.toString()), extraData: extraData })
    ...
    const postStakingBalance = (await getAccount(provider.connection, configData.stakingWallet)).amount;
    const postUserBalance = (await getAccount(provider.connection, userTokenAccount.address)).amount;

    expect(postProgramTokenVaultBalance).to.be.equal(preProgramTokenVaultBalance + transferAmount);
    expect(postStakingBalance).to.be.equal(preStakingBalance + stakingAmount);
  ...
  it("TC-102: deposit success: emit event", async () => {
    ...
    await new Promise(async (resolve) => {
      listener = PaymentContract.addEventListener("Deposit", (eventData) => {
        event = eventData;
        resolve(1);
      });
      await expectIxToSucceed(depositIx, [GPU_RENTER_ACCOUNT_KEYPAIR]).catch(() => {
        resolve(0)
      });
    });
    ...
    expect(event.from.toString()).eq(GPU_RENTER_ACCOUNT_KEYPAIR.publicKey.toString());
  ...
  it("TC-103: deposit fail: zero amount", async () => {
    const { instruction: depositIx } = await deposit(PaymentContract, GPU_RENTER_ACCOUNT_KEYPAIR.publicKey, { amountAitech: new BN(0), extraData: Buffer.from([]) })
    await expectIxToFailWithError(depositIx, "ZeroAmount", [GPU_RENTER_ACCOUNT_KEYPAIR]);
  });
  ...
  it("TC-104: deposit fail: exceed allowance amount", async () => {
    ...
    const { instruction: depositIx } = await deposit(PaymentContract, GPU_RENTER_ACCOUNT_KEYPAIR.publicKey, { amountAitech: depositAmount, extraData: Buffer.from([]) })
    await expectIxToFailWithError(depositIx, "", [GPU_RENTER_ACCOUNT_KEYPAIR]);
  });
  ...
  it("TC-105: deposit fail: paused contract", async () => {
    ...
    const { instruction: depositIx } = await deposit(PaymentContract, GPU_RENTER_ACCOUNT_KEYPAIR.publicKey, { amountAitech: new BN(10000), extraData: Buffer.from([]) })
    await expectIxToFailWithError([pauseIx, depositIx], "Paused", [OWNER_KEYPAIR, GPU_RENTER_ACCOUNT_KEYPAIR]);
  });
});
