...
describe("01. Payment:settings test", async function () {
    ...
    describe("Initialize", () => {
        it("TC-057: constructor invalid feeWallet", async () => {
            ...
            expectIxToFailWithError(initializeIx, "InvalidAddress", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-063: initialize success", async () => {
            ...
            expect(configData.stakingRate).to.be.equal(
                initializeArgs.stakingRate
            );
            expect(configData.burnRate).to.be.equal(initializeArgs.burnRate);
            expect(configData.minimumWithdraw.toString()).to.be.equal(
                initializeArgs.minimumWithdraw.toString()
            );
            expect(configData.maximumWithdraw.toString()).to.be.equal(
                initializeArgs.maximumWithdraw.toString()
            );
            expect(configData.owner.toString()).to.be.equal(
                initializeArgs.owner.toString()
            );
            expect(configData.withdrawSigner.toString()).to.be.equal(
                initializeArgs.withdrawSigner.toString()
            );
        });
    ...
    describe("setBurnRate", () => {
        ...
        it("TC-002: setBurnRate fail when not pause contract", async () => {
            ...
            await expectIxToFailWithError(setBurnRateIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
    ...
    describe("setStakingRate", () => {
        ...
        it("TC-006: stakingRate fail when not pause contract", async () => {
            ...
            await expectIxToFailWithError(setStakingRateIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
    ...
    describe("setFeeRate", () => {
        ...
        it("TC-010: setFeeRate fail when not pause contract", async () => {
            ...
            await expectIxToFailWithError(setFeeRateIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
    ...
    describe("Event Check", () => {
        ...
        it("TC-016: update configuration", async () => {
            ...
            listener = await Promise.all([
                PaymentContract.addEventListener(
                    "FeeRateChange",
                    (eventData) => {
                ...
                PaymentContract.addEventListener(
                    "MinimumWithdrawChange",
                    (eventData) => {
                        event = Object.assign(event, eventData);
                    }
                ),
                PaymentContract.addEventListener(
                    "MaximumWithdrawChange",
                    (eventData) => {
                        event = Object.assign(event, eventData);
                    }
                ),
            ...
            expect(event.newMinimumWithdraw.toString()).to.be.equal(
                newArgs.minimumWithdraw.toString()
            );
            expect(event.newMaximumWithdraw.toString()).to.be.equal(
                newArgs.maximumWithdraw.toString()
            );
            expect(event.newFeeWallet.toString()).to.be.equal(
                newArgs.feeWallet.toString()
            );
            expect(event.newStakingWallet.toString()).to.be.equal(
                newArgs.stakingWallet.toString()
            );
            ...
            expect(configData.feeWallet.toString()).to.be.equal(
                newArgs.feeWallet.toString()
            );
            expect(configData.stakingWallet.toString()).to.be.equal(
                newArgs.stakingWallet.toString()
            );
        });
    ...
    describe("setAiTech", () => {
        ...
        it("TC-019: setToken should fail when contract not pause", async () => {
            const { instruction: setAiTechTokenIx } = await setToken(
                PaymentContract,
                OWNER_KEYPAIR.publicKey,
                { newAitechToken: newAitechToken.publicKey }
            );
            expectIxToFailWithError(setAiTechTokenIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-022: setAitech should success", async () => {
            ...
            await expectIxToSucceed(
                [pauseIx, setAiTechTokenIx],
                [OWNER_KEYPAIR]
            );

            configData = await PaymentContract.account.config.fetch(configPda);
            expect(configData.aitechToken.toString()).to.be.equal(
                newAitechToken.publicKey.toString()
            );

            // reset back to main Aitech token
            const { instruction: resetAitechTokenIx } = await setToken(
                PaymentContract,
                OWNER_KEYPAIR.publicKey,
                { newAitechToken: AITECH_TOKEN.publicKey }
            );
            const { instruction: unpauseIx } = await unpause(
                PaymentContract,
                OWNER_KEYPAIR.publicKey
            ...
            await expectIxToSucceed(
                [resetAitechTokenIx, unpauseIx],
                [OWNER_KEYPAIR]
            );

            configData = await PaymentContract.account.config.fetch(configPda);
            expect(configData.aitechToken.toString()).to.be.equal(
                AITECH_TOKEN.publicKey.toString()
            );
        });
    });
    ...
    describe("setWithdrawSigner", () => {
        it("TC-027: setWithdrawSigner should fail when contract not pause", async () => {
            const { instruction: setWithdrawSignerIx } =
                await setWithdrawSigner(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { withdrawSigner: WITHDRAW_SIGNER_KEYPAIR.publicKey }
                );
            expectIxToFailWithError(setWithdrawSignerIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
    ...
    describe("setMinimumWithdraw", () => {
        it("TC-031: setMinimumWithdraw should fail when contract not pause", async () => {
            const { instruction: setMinimumWithdrawIx } =
                await setMinimumWithdraw(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { minimumWithdraw: new BN("1000000000") }
                );
            expectIxToFailWithError(setMinimumWithdrawIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-032: setMinimumWithdraw should fail when newMinimumWithdraw > maximum", async () => {
            ...
            const { instruction: setMinimumWithdrawIx } =
                await setMinimumWithdraw(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { minimumWithdraw: configData.maximumWithdraw.addn(100) }
                );
            const { instruction: unpauseIx } = await unpause(
                PaymentContract,
                OWNER_KEYPAIR.publicKey
    ...
    describe("setMaximumWithdraw", () => {
        it("TC-035: setMaximumWithdraw should fail when contract not pause", async () => {
            const { instruction: setMaximumWithdrawIx } =
                await setMaximumWithdraw(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { maximumWithdraw: new BN("1000000000") }
                );
            expectIxToFailWithError(setMaximumWithdrawIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-036: setMaximumWithdraw should fail when newMaximumWithdraw < minimum", async () => {
            ...
            const { instruction: setMaximumWithdrawIx } =
                await setMaximumWithdraw(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { maximumWithdraw: configData.minimumWithdraw.subn(100) }
                );
            const { instruction: unpauseIx } = await unpause(
                PaymentContract,
                OWNER_KEYPAIR.publicKey
    ...
    describe("setStakingWallet", () => {
        ...
        it("TC-039: setStakingWallet should fail when contract not pause", async () => {
            const { instruction: setStakingWalletIx } = await setStakingWallet(
                PaymentContract,
                OWNER_KEYPAIR.publicKey,
                { stakingWallet: newStakingWallet.address }
            );
            expectIxToFailWithError(setStakingWalletIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-042: setStakingWallet should success", async () => {
            ...
            await expectIxToSucceed(
                [pauseIx, setStakingWalletIx],
                [OWNER_KEYPAIR]
            );
            configData = await PaymentContract.account.config.fetch(configPda);
            expect(configData.stakingWallet.toString()).to.be.equal(
                newStakingWallet.address.toString()
            );

            // reset
            const { instruction: resetStakingWalletIx } =
                await setStakingWallet(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { stakingWallet: stakingWallet.address }
                );
            ...
            await expectIxToSucceed(
                [resetStakingWalletIx, unpauseIx],
                [OWNER_KEYPAIR]
            );

            configData = await PaymentContract.account.config.fetch(configPda);
            expect(configData.stakingWallet.toString()).to.be.equal(
                stakingWallet.address.toString()
            );
        });
    });
    ...
    describe("setFeeWallet", () => {
        ...
        it("TC-047: setFeeWallet should fail when contract not pause", async () => {
            const { instruction: setFeeWalletIx } = await setFeeWallet(
                PaymentContract,
                OWNER_KEYPAIR.publicKey,
                { feeWallet: newFeeWallet.address }
            );
            expectIxToFailWithError(setFeeWalletIx, "NotPaused", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-050: setFeeWallet should success", async () => {
            ...
            configData = await PaymentContract.account.config.fetch(configPda);
            expect(configData.feeWallet.toString()).to.be.equal(
                newFeeWallet.address.toString()
            );

            // reset
            const { instruction: resetFeeWalletIx } = await setFeeWallet(
                PaymentContract,
                OWNER_KEYPAIR.publicKey,
                { feeWallet: feeWallet.address }
            );
            ...
            await expectIxToSucceed(
                [resetFeeWalletIx, unpauseIx],
                [OWNER_KEYPAIR]
            );

            configData = await PaymentContract.account.config.fetch(configPda);
            expect(configData.feeWallet.toString()).to.be.equal(
                feeWallet.address.toString()
            );
        });
    });
    ...
    describe("setPaused", () => {
        it("TC-051: setPaused should fail cause caller is not owner", async () => {
            ...
            await expectIxToFailWithError(pauseIx, "NotOwner", [
                REST_KEYPAIR[0],
            ]);
        });
    });
    ...
    describe("emergencyWithdraw", () => {
        it("TC-052: emergencyWithdraw should fail cause caller is not owner", async () => {
            const { instruction: emergencyWithdrawIx } =
                await emergencyWithdraw(
                    PaymentContract,
                    REST_KEYPAIR[0].publicKey,
                    { amountAitech: new BN("1000000000") }
                );
            expectIxToFailWithError(emergencyWithdrawIx, "NotOwner", [
                REST_KEYPAIR[0],
            ]);
        });
        ...
        it("TC-053: emergencyWithdraw should fail amount = 0", async () => {
            const { instruction: emergencyWithdrawIx } =
                await emergencyWithdraw(
                    PaymentContract,
                    OWNER_KEYPAIR.publicKey,
                    { amountAitech: new BN("0") }
                );
            expectIxToFailWithError(emergencyWithdrawIx, "ZeroAmount", [
                OWNER_KEYPAIR,
            ]);
        });
        ...
        it("TC-055: emergencyWithdraw success AITECH", async () => {
            ...
            const preOwnerAitechBalance = await getAccount(
                provider.connection,
                ownerAitechTokenAccount.address
            );
            const { instruction: emergencyWithdrawIx } =
