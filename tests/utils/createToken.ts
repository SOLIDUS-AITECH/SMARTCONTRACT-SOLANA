...
export async function createToken(connection: Connection) {
    ...
    const ownerAitechAta = await getOrCreateAssociatedTokenAccount(
        connection,
        OWNER_KEYPAIR,
        AITECH_TOKEN.publicKey,
        OWNER_KEYPAIR.publicKey
    );
    const gpuOwnerAitechAta = await getOrCreateAssociatedTokenAccount(
        connection,
        OWNER_KEYPAIR,
        AITECH_TOKEN.publicKey,
        GPU_OWNER_ACCOUNT_KEYPAIR.publicKey
    );
    const gpuRenterAitechAta = await getOrCreateAssociatedTokenAccount(
        connection,
        OWNER_KEYPAIR,
        AITECH_TOKEN.publicKey,
    ...
    const ownerUsdtAta = await getOrCreateAssociatedTokenAccount(
        connection,
        OWNER_KEYPAIR,
        USDT_TOKEN.publicKey,
        OWNER_KEYPAIR.publicKey
    );
    const gpuOwnerUsdtAta = await getOrCreateAssociatedTokenAccount(
        connection,
        OWNER_KEYPAIR,
        USDT_TOKEN.publicKey,
        GPU_OWNER_ACCOUNT_KEYPAIR.publicKey
    );
    const gpuRenterUsdtAta = await getOrCreateAssociatedTokenAccount(
        connection,
        OWNER_KEYPAIR,
        USDT_TOKEN.publicKey,
        GPU_RENTER_ACCOUNT_KEYPAIR.publicKey
