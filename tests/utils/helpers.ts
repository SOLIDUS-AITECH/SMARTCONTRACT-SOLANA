...
export const boilerPlateReduction = (
    ...
    const expectIxToSucceed = async (
        ...
        expect(
            sendAndConfirmIx(ix, signerOrSignersOrComputeUnits, computeUnits)
        ).to.be.fulfilled;
    ...
    const expectIxToFailWithError = async (
        ...
        try {
            ...
            const regex = /Error Message:\s*(.+)/;
            const match = (error as SendIxError).logs.match(regex);
            if (match) {
    ...
    const expectTxToSucceed = async (
        ...
        return expect(sendAndConfirmTransaction(connection, await tx, [wallet]))
            .to.be.fulfilled;
    };
...
export const sleepSeconds = (s: number) =>
    new Promise((f) => setTimeout(f, s * 1000));
